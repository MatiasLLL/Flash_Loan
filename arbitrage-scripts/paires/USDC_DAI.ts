...
import { arbitrageContractWithSigner, provider, quickSwapAddress, sushiSwapAddress, usdcAddress, daiAddress, quickSwapContractWithGetAmountsOut, sushiSwapContractWithGetAmountsOut, exchangeAtIndex, tokenAtIndex, tokenToBorrow } from '../utils';

export async function Pair1() {
  const amountIn = ethers.parseUnits('1', 6);
  const amountInDAI = ethers.parseUnits('1', 18);

  try {
    // Get amount QuickSwap
    const amountsOutQuickSwap1 = await quickSwapContractWithGetAmountsOut.getAmountsOut(amountIn, [usdcAddress, daiAddress]);
    const amountsOutQuickSwap2 = await quickSwapContractWithGetAmountsOut.getAmountsOut(amountInDAI, [daiAddress, usdcAddress]);
    // Get amount SushiSwap
    const amountsOutSushiSwap1 = await sushiSwapContractWithGetAmountsOut.getAmountsOut(amountIn, [usdcAddress, daiAddress]);
    const amountsOutSushiSwap2 = await sushiSwapContractWithGetAmountsOut.getAmountsOut(amountInDAI, [daiAddress, usdcAddress]);
    // Get price QuickSwap
    const priceQuickSwap_usdcTOdai = BigInt(amountsOutQuickSwap1[1]) / BigInt(1e12);
    const priceQuickSwap_daiTOusdc = BigInt(amountsOutQuickSwap2[1]);
    // Get price SushiSwap
    const priceSushiSwap_usdcTOdai = BigInt(amountsOutSushiSwap1[1]) / BigInt(1e12);
    const priceSushiSwap_daiTOusdc = BigInt(amountsOutSushiSwap2[1]);

    const arrayPrice = [
      priceQuickSwap_usdcTOdai, priceQuickSwap_daiTOusdc,
      priceSushiSwap_usdcTOdai, priceSushiSwap_daiTOusdc
    ];
    ...
    const firstSwapToken = tokenAtIndex(firstSwapIndex, "USDC_DAI");
    const secondSwapToken = tokenAtIndex(secondSwapIndex, "USDC_DAI");
    const firstSwapDex = await exchangeAtIndex(firstSwapIndex);
    const secondSwapDex = await exchangeAtIndex(secondSwapIndex);
    const borrowToken = tokenToBorrow(firstSwapIndex, "USDC_DAI"); 
    
    const firstSwapPath = firstSwapToken === 'USDC_to_DAI' ? [usdcAddress, daiAddress] : firstSwapToken === 'DAI_to_USDC' ?  [daiAddress, usdcAddress] : [];
    const secondSwapPath = (borrowToken === 'USDC') ? [daiAddress, usdcAddress] : [usdcAddress, daiAddress];
    ...
    const addressTokenBorrow: string = (borrowToken === 'USDC' ? usdcAddress : borrowToken === 'DAI' ? daiAddress : '') as string;
    const decimals = borrowToken === 'USDC' ? 6 : borrowToken === 'DAI' ? 18 : 0;
...
// setInterval(Pair1, 20000); // every 20 seconds


// const secondSwapPath  = secondSwapToken === 'USDC_to_DAI' ? [usdcAddress, daiAddress] : secondSwapToken === 'DAI_to_USDC' ?  [daiAddress, usdcAddress] : [];
// const firstSwapPath = (firstSwapToken === 'USDC_to_DAI') ? [usdcAddress, daiAddress] : [daiAddress, usdcAddress];
